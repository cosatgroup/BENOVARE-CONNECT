...
export default function PrestataireMissionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [mission, setMission] = useState<PrestataireMission | null>(null);
  ...
  const [rechDescription, setRechDescription] = useState("");
  ...
  async function handleRechercheTalents(e: React.FormEvent) {
    e.preventDefault();
    if (!rechTitre.trim() || !rechDescription.trim()) return;
    try {
      await rechercherTalents(id, rechTitre, rechDescription, mission!.besoin.niveauEtoiles);
      setRechTitre("");
      setRechDescription("");
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <section>
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-neutre-600">
            Rechercher des talents pour cette mission
          </h2>
          <button onClick={() => setShowRecherche((v) => !v)} className="text-sm font-medium text-vert-benovare">
            Publier un avis
          </button>
        </div>
        {showRecherche && (
          <form onSubmit={handleRechercheTalents} className="mt-3 space-y-2 rounded-lg border border-neutre-200 bg-surface p-4">
            <input
              required
              placeholder="Titre de l'avis (ex. Consultant DevOps — renfort d'équipe)"
              value={rechTitre}
              onChange={(e) => setRechTitre(e.target.value)}
              className="w-full rounded-md border border-neutre-200 px-3 py-2 text-sm"
            />
            <textarea
              required
              placeholder="Description du besoin…"
              value={rechDescription}
              onChange={(e) => setRechDescription(e.target.value)}
              rows={3}
              className="w-full rounded-md border border-neutre-200 px-3 py-2 text-sm"
            />
            <button
      ...
      <section>
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowImprevu((v) => !v)} className="text-sm font-medium text-vert-benovare">
            Déclarer un imprévu
          </button>
        </div>
